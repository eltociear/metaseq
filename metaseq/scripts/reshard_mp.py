...
def reshard_model_parallel_parts(
    ...
    are merged from the parts before the resharding logic applies. The MP parts are
    expected to contain unflattened, FSDP-consolidated model weights (see the script
    `reshard_fsdp.py` for related information.)
    ...
    Args:
        ...
        :param eps: A tolerance threshold for the maxinum discrepancy between MP parts.
    """
    files = glob(input)
